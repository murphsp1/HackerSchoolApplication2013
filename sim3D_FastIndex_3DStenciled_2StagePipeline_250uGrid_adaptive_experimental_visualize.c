...
//MUST COMMENT THIS BACK IN TO WRITE OUT DATA
#define BZ_IMPORT

#define CHUNK 5
//#define NUMLOOPS 11318
#define NUMLOOPS 150000
...
#elif MODERATE //MODERATE ISCHEMIA
...
#elif MILD //MILD ISCHEMIA
#define ISCHEMIC_END_DMN 145			/* Minimum diastolic interval */
#define ISCHEMIC_END_DMD 245			/* diastolic iterval where line and exponent meet */
#define ISCHEMIC_END_DMX 250 		/* Maximum diastolic interval */
#elif HEALTHY
#define ISCHEMIC_END_DMN	25			/* Minimum diastolic interval */
#define ISCHEMIC_END_DMD 125			/* diastolic iterval where line and exponent meet */
#define ISCHEMIC_END_DMX 250 		/* Maximum diastolic interval */
#endif
...
#elif MILD//MILD ISCHEMIA
#define ISCHEMIC_EPI_DMN 145			/* Minimum diastolic interval */
#define ISCHEMIC_EPI_DMD 245			/* diastolic iterval where line and exponent meet */
#define ISCHEMIC_EPI_DMX 250			/* Maximum diastolic interval */
#elif HEALTHY
#define ISCHEMIC_EPI_DMN	25			/* Minimum diastolic interval */
#define ISCHEMIC_EPI_DMD 125			/* diastolic iterval where line and exponent meet */
#define ISCHEMIC_EPI_DMX 250			/* Maximum diastolic interval */
#endif
...
#elif MILD//MILD ISCHEMIA
#define ISCHEMIC_MID_DMN 145			/* Minimum diastolic interval */
#define ISCHEMIC_MID_DMD 245			/* diastolic iterval where line and exponent meet */
#define ISCHEMIC_MID_DMX 250			/* Maximum diastolic interval */
#elif HEALTHY
#define ISCHEMIC_MID_DMN	25			/* Minimum diastolic interval */
#define ISCHEMIC_MID_DMD 125			/* diastolic iterval where line and exponent meet */
#define ISCHEMIC_MID_DMX 250			/* Maximum diastolic interval */
#endif
...
int main( void )
{
	...
	int intBeatCounter = 0;
	int intInterBeatInterval = 199;
	int intLastBeat = 0;
	int intBPM = 135;
	char fname[14];
...
*/
 ...
 /* Open ECG file */
	...
	sprintf(fname,"ECG_%iBPM.dat",intBPM);
	ecg = fopen(fname, "w");
...
/*
		...
		if (  (intBeatCounter<3) ) {
			...
			if ( ( (tm-intLastBeat)% intInterBeatInterval) == 0) {
				purkinje_fibre_stim_human3();
				printf("STIMULUS APPLIED, tm = %i\n",tm);
				intLastBeat = tm;
				intBeatCounter = 0;
				intInterBeatInterval = intInterBeatInterval - 10;
			}
		}
...
*/
		...
		if (intBeatCounter == 2) {
			intBeatCounter = 0;
			intInterBeatInterval = intInterBeatInterval - 3;
		}
		
		//*/
		 /*GOOD PACING
		//This paces the heart a given number of times at a certain BPM (intBPM)
		intInterBeatInterval = (int)( 1.0/(float)intBPM * 60.0 * 1000.0/2.0) ;

		if ( ( (tm-intLastBeat)% intInterBeatInterval) == 0) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i, BPM = %i,  IBI = %i\n",tm,intBPM,intInterBeatInterval);
			intLastBeat = tm;
			intBeatCounter++;
		}

		if (intBeatCounter > 40) {
			printf("tm = %i\n",tm);
			break;
		}
		*/
...
static float ischemic_apd_from_restitution (unsigned short tau, int cellType)
{
	...
	if (cellType == ENDOCARDIAL_CELL) {
		...
		if (di < ISCHEMIC_END_DMN) {
			...
			apd = ISCHEMIC_END_MIN + ischemic_apd_end_slp*(di-ISCHEMIC_END_DMN);
		} else {
			apd = ISCHEMIC_END_MID + (ISCHEMIC_END_MAX-ISCHEMIC_END_MID)*(1 - exp(-ischemic_apd_end_exp*(di - ISCHEMIC_END_DMD)));
		}
		...
		if (di < ISCHEMIC_EPI_DMN) {
			...
			apd = ISCHEMIC_EPI_MIN + ischemic_apd_epi_slp*(di-ISCHEMIC_EPI_DMN);
		} else {
			apd = ISCHEMIC_EPI_MID + (ISCHEMIC_EPI_MAX-ISCHEMIC_EPI_MID)*(1 - exp(-ischemic_apd_epi_exp*(di - ISCHEMIC_EPI_DMD)));
		}
		...
		if (di < ISCHEMIC_MID_DMN) {
			...
			apd = ISCHEMIC_MID_MIN + ischemic_apd_mid_slp*(di-ISCHEMIC_MID_DMN);
		} else {
			apd = ISCHEMIC_MID_MID + (ISCHEMIC_MID_MAX-ISCHEMIC_MID_MID)*(1 - exp(-ischemic_apd_mid_exp*(di - ISCHEMIC_MID_DMD)));
		}
	}
	return (apd);
}
