...
typedef struct cam_pde_element {
	...
	unsigned char  flags;		/* various flags condensed into one variable
						bit1: (boundry)	Is volume element on boundry	
...
#elif HUMAN
void initialize_geometry_sj(void);
...
/* Loop over the total number of time steps */
	for (tm = 0; tm < NUMLOOPS; tm++) {
		...
		fprintf(ecg, "%lf\t%lf\t%lf\n", fTime , ecgLead, V3_ecgLead);
		fflush(ecg);
		printf("time = %lf (ms) , ECG = %lf , ECG = %lf\n", tm*2.0 , ecgLead, V3_ecgLead);		
		ecgLead = 0.0;
		V3_ecgLead = 0.0;
	 ...
	 /* Pacing the Heart at 60 bpm (tm%500)*/
		...
		*/
		...
		} else if ( (tm >= 2500) && (tm < 5000) && ((tm % 353)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 5000) && (tm < 7500) && ((tm % 333)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 7500) && (tm < 10000) && ((tm % 316)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 10000) && (tm < 12500) && ((tm % 286)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 12500) && (tm < 15000) && ((tm % 273)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 15000) && (tm < 17500) && ((tm % 261)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 17500) && (tm < 20000) && ((tm % 250)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 20000) && (tm < 22500) && ((tm % 240)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		} else if ( (tm >= 22500) && (tm < 25000) && ((tm % 231)==0)) {
			purkinje_fibre_stim_human3();
			printf("STIMULUS APPLIED, tm = %i\n",tm);
		}
//		continue;
		...
		/*
		if ((tm/1000) < 5)
			purkinje_fibre_stim_human3(tm % 250);
		else if ((tm/1000) < 10)
			purkinje_fibre_stim_human3(tm % 300);
		else if ((tm/1000) < 30)
			purkinje_fibre_stim_human3(tm % 500);
		*/
